...
//        NSParameterAssert([args[0] isKindOfClass:[SNSymbol class]]);
        ...
        [env setObject:value forKey:args[0]];
        return value;
    };
}
...
#pragma mark - Runtime
+ (id)lookup:(NSString *)symbol in:(NSDictionary *)dictionary
{
    if ([[NSCharacterSet uppercaseLetterCharacterSet] characterIsMember:[symbol characterAtIndex:0]]) {
        return NSClassFromString(symbol) ?: symbol;
    } else {
        return [dictionary objectForKey:symbol] ?: symbol;
    }
}
...
{
    return [expression isKindOfClass:[NSString class]] ? [self lookup:expression in:context]
...
{
    return [self deconstruct:array withBlock:^id(NSString *head, NSArray *arguments) {
#ifdef DEBUG
        //        NSParameterAssert([head isKindOfClass:[SNSymbol class]]);
#endif
        SNProcedure proc = [self evaluate:head inContext:context];
        return [head isKindOfClass:[NSString class]] && [[self specialFormSymbols] containsObject:head]
